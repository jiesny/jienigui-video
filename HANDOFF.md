...
- 搜索框保持普通输入框形态；聚焦/输入不自动展开历史，只有点击左侧小时钟历史按钮才显示历史。
- 支持点击复搜、单条删除、清空全部。
- 搜索记录下拉层位于顶部导航内，层级必须高于内容卡片，避免被页面框架遮住。
...
- 纯白花嫁使用纯白、柔粉、淡金提示风格。
- 透明玻璃为全端静态主题，不使用背景或卡片流动动画；仍保留淡蓝白玻璃模糊质感。
- 移动端顶部布局为 logo、搜索框、设置键一行。
- 全局滚动条默认隐藏，不预留滚动槽，避免加载状态下滚动条出现/消失造成页面左右晃动。
- 竖屏详情页播放器占满屏宽，详情内容在下方滚动；详情面板使用受控 overflow、隐藏详情侧栏滚动条、长文本强制换行、自适应集数网格，避免长标题/长集名或浅色主题滚动槽撑出右侧边框。
- 横屏详情页为左侧播放器、右侧详情栏。
